this.reorder = new function(){
  ...
  this.pushQueue = function(i, data, el){
    ...
    let summInventories = 0;
    data.forEach(obj => summInventories = summInventories + (obj.inventory === "" ?  0 :  +obj.inventory) );

    if(summInventories > 0){
      ...
      el.removeClass('loading');
      $(".errorMessege").removeAttr("hidden");
    } 
  }
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        if (!newVariant) {
          ...
          // Sur: For additional styles
          addToCartButtonElement.classList.add('product-form__button-unavailable')
        } else {
          ...
          if (newVariant['available']) {
            ...
            // Sur: Remove styles
            addToCartButtonElement.classList.remove('product-form__button-unavailable');
          } else {
            addToCartButtonElement.setAttribute('disabled', 'disabled');
            addToCartButtonElement.classList.add('button--disabled');
            addToCartButtonElement.classList.remove('button--primary');
  ...
  */
    ...
    _createClass(CollectionSection, [{
      ...
      value: function _paginationPageChanged(event, target) {
        ...
        if(!location.pathname.includes("blogs/")) {
            if (target.getAttribute('data-page') != '1' ) {
              ...
              this.currentUrl.searchParams.set('page', parseInt(target.getAttribute('data-page')));	
               ...
               window.location.href = this.currentUrl.pathname;
            }
        } else {
          this.currentUrl.searchParams.set('page', parseInt(target.getAttribute('data-page')));
        }
  ...
  (function () {
    var bootTheme = function bootTheme() {
      ...
      sections.register('rewards', AccountSection);
      ...
      (function () {
        ...
        fastdom.mutate(function () {
          for (var i = 0, linksLength = links.length; i < linksLength; i++) {
            if (links[i].hostname !== window.location.hostname && links[i].hostname !== 'teen.smokefree.gov') {
              links[i].target = '_blank';
              if(links[i].getAttribute('data-rel-static')) {
              	links[i].rel = 'noopener sponsored';
              } else {
